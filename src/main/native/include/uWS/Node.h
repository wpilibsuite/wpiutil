...
private:
    ...
    static void accept_cb(ListenSocket *listenSocket) {
        ...
        do {
            Socket *socket = new Socket(listenSocket->nodeData, listenSocket->nodeData->loop, clientFd, nullptr);
...
public:
    ...
    Socket *connect(const char *hostname, int port, bool secure, NodeData *nodeData) {
        ...
        Socket initialSocket(nodeData, getLoop(), fd, nullptr);
    ...
    template <void A(Socket *s)>
    bool listen(const char *host, int port, void* sslContext, int options, uS::NodeData *nodeData, void *user) {
        addrinfo hints, *result;
        memset(&hints, 0, sizeof(addrinfo));
