...
public:
    Socket(NodeData *nodeData, Loop *loop, uv_os_sock_t fd, void *ssl) : Poll(loop, fd), nodeData(nodeData) {
    }
...
struct ListenSocket : Socket {
    ...
    ListenSocket(NodeData *nodeData, Loop *loop, uv_os_sock_t fd, void *ssl) : Socket(nodeData, loop, fd, ssl) {
