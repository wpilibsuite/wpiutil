...
static void base64(const unsigned char *src, char *dst) {
...
void HttpSocket<isServer>::upgrade(const char *secKey, const char *extensions, size_t extensionsLength,
    ...
    if (isServer) {
        *perMessageDeflate = false;
        ...
        char shaInput[] = "XXXXXXXXXXXXXXXXXXXXXXXX258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
        memcpy(shaInput, secKey, 24);
        wpi::SHA1 sha;
        sha.Update(shaInput);
        llvm::SmallString<32> shaDigestBuf;
        llvm::StringRef shaDigest = sha.Final(shaDigestBuf);

        char upgradeBuffer[1024];
        memcpy(upgradeBuffer, "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: ", 97);
        base64(reinterpret_cast<const unsigned char*>(shaDigest.data()), upgradeBuffer + 97);
